...
@section('title')
    @lang('translation.coldEmailingRules.editColdEmailingRules')
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('cold-emailing-rules.update', ['id' => $model->id]) }}" method="POST">
                        @method('PUT')
                        {{ csrf_field() }}
                            ...
                            <div class="col-3 ">
                                ...
                                <select class="form-control" name="cold_emailing_credentials_id" id="cold_emailing_credentials_id_dropdown">
                                    @foreach($coldEmailingCredentials as $value)
                                        <option value="{{ $value->id }}" {{ (old('cold_emailing_credentials_id') == $value->id || (isset($model) && $model->cold_emailing_credentials_id == $value->id)) ? 'selected' : '' }}>{{ $value->email }}</option>
                                    @endforeach
                                </select>
