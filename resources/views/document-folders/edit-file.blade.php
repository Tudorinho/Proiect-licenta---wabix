...
@section('title')
    @lang('translation.documentFolders.editFile')
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('document-folders.update-file', ['id'=>$file->id]) }}" method="POST">
                        @method('PUT')
                        {{ csrf_field() }}
						<h5 class="mt-3 mb-3">@lang('translation.headings.generalInformation')</h5>

						<div class="row">

                            {{-- document_folder_id --}}
                            {{-- <input type="hidden" name="document_folder_id" value="{{request()->input('document_folder_id')}}"> --}}
                            <input type="hidden" name="document_folder_id" value="{{ $file->document_folder_id }}">
