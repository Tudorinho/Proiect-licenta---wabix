...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('worklogs.update', ['id' => $model->id]) }}" method="POST">
                        @method('PUT')
                        {{ csrf_field() }}
                            ...
                            <div class="col-3">
                                ...
                                <select class="form-control" name="employee_id" id="employee_id_dropdown">
                                    @foreach($employees as $value)
                                        <option value="{{ $value->id }}" {{ (old('employee_id') == $value->id || (isset($model) && $model->employee_id == $value->id)) ? 'selected' : '' }}>{{ $value->first_name.' '.$value->last_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-3 ">
                                ...
                                <select class="form-control" name="project_id" id="project_id_dropdown">
                                    @foreach($projects as $value)
                                        <option value="{{ $value->id }}" {{ (old('project_id') == $value->id || (isset($model) && $model->project_id == $value->id)) ? 'selected' : '' }}>{{ $value->name }}</option>
                                    @endforeach
                                </select>
